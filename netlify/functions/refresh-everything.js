...
exports.handler = async (event) => {
  ...
  try {
    ...
    await dispatchWorkflow("draft-queue.yml", token);

    return json(200, { ok: true, triggered: ["daily-pulse.yml", "standup.yml", "draft-queue.yml"], triggeredAt: new Date().toISOString() });
  } catch (err) {
